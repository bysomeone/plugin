...
不可逆）；B3 丢 = 麻烦（可自愈 + 需重扫，服务降级）；B4 丢 = 看拓扑（单主节点 = 完蛋；多主节点 = 麻烦）。
...
> ⚠️ **推断**：本轮未核实生产主链拓扑（compose 是 CI 用的）。上面的结论只在「生产沿用单主节点」
> 时成立。请把这条当**待确认的架构问题**，而不是既成事实。
...
| **G6** | **生产主链拓扑未确认**（CI 是单主节点） | 若生产也是单主节点 ⇒ `main-data` 丢 = 链丢 = 全部合约状态丢 | **需架构定夺，非工作量问题** | 用户决策 |
