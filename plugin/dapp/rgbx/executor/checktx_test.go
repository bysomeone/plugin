...
func Test_checkConfirm(t *testing.T) {
	...
	tcArr := []*testCase{
		...
		{
			// 尾随字节可塑性（A2/E1）：给同一笔花费追加尾部字节后，解析结果与 txid 都不变，
			// 但原始字节变了 —— 必须被拒，否则"归属 utxo id / OP_RETURN 承诺"的判据会随编码漂移。
			// 上游把内容判据统一改走 merkle 认证过的 BtcTxProof（validateBtcTxProof 拒尾随字节），
			// 故这里期望 ErrInvalidBtcTxProof（旧码 ErrNonCanonicalSpendingTx 已无引用）。
			// 该用例不需要区块头 mock：尾随字节在 getBtcHeader 之前就被拒。
			expectErr: ErrInvalidBtcTxProof,
			action: &rtypes.ConfirmTx{UtxoProof: &rtypes.UtxoSpendingProof{}, BtcTxProof: &rtypes.BtcTxProof{
				TxData: append(append([]byte{}, buf.Bytes()...), 0x00),
			}},
		},
		{
			// 解不出来的证明字节 ⇒ ErrInvalidBtcTxProof（上游新增的形态）。
			expectErr: ErrInvalidBtcTxProof,
	...
	btcProofCases := []*testCase{
		...
		{
			// 证明有效、输入也匹配，但 opRetOutIdx 越界（该 utxo 已在 btc 链上花费却没有
			// rgbx 约束的 OP_RETURN 输出）⇒ 资产冻结：checkConfirm 放行，Exec_Confirm 只做标记。
			expectErr: nil,
			action: &rtypes.ConfirmTx{
				TxBlockHeight: 10,
				TxIndex:       0,
