...
// TestRgbx_Exec_Confirm Exec 侧的 mint/transfer 确认结算。
//
// 判据全部取自 **BtcTxProof.TxData** 这一份 merkle 认证过的交易（UtxoProof.SpendingTx 与
// OpRetOutputPkScript 两个字段上游已删除）：Exec_Confirm 解析它拿 txid 作为归属 utxo id，
// 并用"opRetOutIdx 处的输出 == NullDataScript(confirm.TxHash)"判定承诺成立。
// merkle/区块头的校验在 CheckTx 侧（validateBtcTxProof，见 checktx_test.go 的 Test_checkConfirm），
// Exec 层直接构造交易字节即可。
...
//
// 注：走生产路径时这类编码在 CheckTx 就被拒了（validateBtcTxProof 拒尾随字节，
// 见 Test_checkConfirm 的 ErrInvalidBtcTxProof 用例）；本用例直接调 Exec_Confirm，
// 证明 **spendHash 的推导本身** 与编码无关（不依赖上游那道门）。
