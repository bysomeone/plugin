...
require (
	...
	github.com/getamis/alice v1.0.7 // indirect
...
// 临时脚手架（fork-until-upstream）：chain33 的 CGGMP 包只存在于 fork 分支
// feat/tss-alice-cggmp 上 —— 上游 chain33 v1.72.0 只有 tss/gg18，alice 还停在 v1.0.3，
// 而本分支的 rgbx / lightclient 都 import system/crypto/tss/cggmp。因此**本分支必须钉住
// 这个 fork 才能编译**，这一行不能删（删了 HEAD 就是编不过的）。
// 退出条件：chain33 分支合入上游并发版后，删掉本行、把上面的 require 指到发行版，MVS 自动接上。
// 注意：曾经把它当"本地验证用、不提交"而只在工作区留着，结果 committed 的分支长期编不过、
// 全靠未提交的工作区撑着 —— 别再那样做。
replace github.com/33cn/chain33 => github.com/bysomeone/chain33 v1.72.1-0.20260919083032-36fb0fd21884
